...
CREATE TABLE br_user (
	...
	FOREIGN KEY (account_id) REFERENCES br_user_account(account_id)
);

/* ... AND ADDED TO br_user_account TABLE 
	Additionally, references to br_user's columns of username, password and e-mail
	have been scrubbed from the rest of the script. The following br_user_account is
...
CREATE TABLE br_user_account (
	...
	--FOREIGN KEY (user_id) REFERENCES br_user(user_id)
	);
...
CREATE TRIGGER tr_user_profile_insert_after AFTER INSERT
    ON br_user FOR EACH ROW BEGIN
    INSERT INTO br_bogreden_log (
        log_id,
        log_table_key,
...
/* BEFORE INSERT TRIGGER ADDED FOR TABLE br_user_account */

CREATE TRIGGER tr_user_account_before_insert BEFORE INSERT
	ON br_user_account FOR EACH ROW BEGIN
	INSERT INTO br_bogreden_log (
		log_id,
		log_table_key,
		log_time_stamp,
...
tr_user_account_update_after AFTER UPDATE
ON br_user_account FOR EACH ROW BEGIN
	INSERT INTO br_bogreden_log (
		log_id,
		log_table_key,
...
tr_user_account_update_after BEFORE UPDATE
ON br_user_account FOR EACH ROW BEGIN
	INSERT INTO br_bogreden_log (
		log_id,
		log_table_key,
...
CREATE TRIGGER tr_user_account_delete_after AFTER DELETE
ON tr_user FOR EACH ROW BEGIN
	INSERT INTO br_bogreden_log (
		log_id,
		log_table_key,
		log_time_stamp,
...
CREATE TRIGGER tr_user_account_before_delete BEFORE DELETE
ON br_user_account FOR EACH ROW BEGIN
	INSERT INTO br_bogreden_log (
		log_id,
		log_table_key,
...
BEGIN
    --IF `password` = `confirm_password` THEN
        SET @sql = 'INSERT INTO br_user (user_first_name, user_last_name, user_phone_number) VALUES (?, ?, ?, ?, ?, ?)';  -- user_username, user_password, user_email,
...
BEGIN
    --IF `password` IS NOT NULL THEN
        --IF `password` = `confirm_password` THEN
            SET @sql = 'UPDATE br_user SET user_password = ?, user_first_name = ?, user_last_name = ?, user_phone_number = ? WHERE user_id = ?'; -- user_username = ?, user_email = ?, 
            PREPARE stmt FROM @sql;
