...
/* USER ACCOUNT COLUMNS REMOVED FROM br_user TABLE ... */



/* ... AND ADDED TO br_account TABLE 
	Additionally, references to br_user's columns of username, password and e-mail
	have been scrubbed from the rest of the script. The following br_account is
	meant to replace and separate user data. */
...
CREATE TABLE br_user (
	...
	FOREIGN KEY (user_account_id) REFERENCES br_account(account_id) ON DELETE CASCADE
);
...
CREATE TABLE br_order (
    ...
    FOREIGN KEY (odr_user_id) REFERENCES br_user(user_id) ON DELETE SET NULL
);
...
CREATE INDEX idx_account ON br_account (
    account_id,
    account_username,
    account_password,
    account_email,
...
/* ADDED AFTER INSERT USER ACCOUNT TRIGGER */

CREATE TRIGGER tr_account_insert_after AFTER INSERT
    ON br_account FOR EACH ROW BEGIN
    INSERT INTO br_bogreden_log (
        log_id,
        log_table_key,
        log_time_stamp,
    ...
    VALUES (
        ...
        CURRENT_TIMESTAMP(),
        CONCAT(
			'account_id: ', COALESCE(NEW.account_id), ', ',
            'user_username: ', COALESCE(NEW.account_username,"NULL"), ', ',
            'user_password: ', COALESCE(NEW.account_password,"NULL"), ', ',
            'user_email: ', COALESCE(NEW.account_email,"NULL"), ', ',
            'account_role:', COALESCE(NEW.account_role,"NULL"), ',',', '
        )
    );
END¤
...
/* BEFORE INSERT TRIGGER ADDED FOR TABLE br_account */

CREATE TRIGGER tr_account_before_insert BEFORE INSERT
	ON br_account FOR EACH ROW BEGIN
	INSERT INTO br_bogreden_log (
		log_id,
		log_table_key,
...
CREATE TRIGGER tr_account_update_after AFTER UPDATE
ON br_account FOR EACH ROW BEGIN
	INSERT INTO br_bogreden_log (
		log_id,
		log_table_key,
	...
	( 
		...
		CURRENT_TIMESTAMP(),
		CONCAT(
			'account_id: ', COALESCE(new.account_id, "NULL"), ', ',
			'account_username: ', COALESCE(NEW.account_username, "NULL"), ', ',
			'user_password: ', COALESCE(NEW.account_password, "NULL"), ', ',
            'account_role: ', COALESCE(NEW.account_role, "null")
		)
	);
...
CREATE TRIGGER tr_account_update_before BEFORE UPDATE
ON br_account FOR EACH ROW BEGIN
	INSERT INTO br_bogreden_log (
		log_id,
		log_table_key,
	...
	( 
		...
		CURRENT_TIMESTAMP(),
		CONCAT(
			'account_id: ', COALESCE(OLD.account_id, "NULL"), ', ',
			'account_username: ', COALESCE(OLD.account_username, "NULL"), ', ',
			'user_password: ', COALESCE(OLD.account_password, "NULL"), ', ',
            'account_role: ', COALESCE(OLD.account_role, "NULL"), ', '
		)
	);
END¤
...
CREATE TRIGGER tr_account_delete_after AFTER DELETE
ON br_user FOR EACH ROW BEGIN
	INSERT INTO br_bogreden_log (
		log_id,
		log_table_key,
...
CREATE TRIGGER tr_account_before_delete BEFORE DELETE
ON br_account FOR EACH ROW BEGIN
	INSERT INTO br_bogreden_log (
		log_id,
		log_table_key,
...
-- ------------------------------------- --
/* CREATES A NEW USER ACCOUNT with necessary information. 'Username' in this case may be null, but an e-mail is always required. */

CREATE PROCEDURE insert_new_account(
    IN `username` VARCHAR(50),
    IN `email` VARCHAR(50),
    IN `password` VARCHAR(256),
    IN `confirm_password` VARCHAR(256),
    IN `role` ENUM('Admin', 'Employee', 'Customer'),
    OUT `new_account_id` INT
)
BEGIN
    IF `password` = `confirm_password` THEN
        INSERT INTO br_account (account_id, account_username, account_email, account_password, account_role)
        VALUES (NULL, `username`, `email`, `password`, COALESCE(`role`, 'Customer'));
...
BEGIN
    SET @sql = 'INSERT INTO br_user (user_first_name, user_last_name, user_phone_number, user_account_id) VALUES (?, ?, ?, ?)';
    PREPARE stmt FROM @sql;
    EXECUTE stmt USING `first_name`, `last_name`, `phone_number`, `user_account_id`;
    DEALLOCATE PREPARE stmt;
END¤

CREATE PROCEDURE insert_new_user_account(
    IN `username` VARCHAR(50),
    IN `email` VARCHAR(50),
    IN `password` VARCHAR(256),
    IN `confirm_password` VARCHAR(256),
...
BEGIN
    ...
    CALL insert_new_account(`username`, `email`, `password`, `confirm_password`, `role`, `new_account_id`);
    IF `new_account_id` IS NOT NULL THEN
        CALL insert_new_user(`first_name`, `last_name`, `phone_number`, `new_account_id`);
    END IF;
END¤
...
BEGIN
    -- IF `password` IS NOT NULL THEN
        -- IF `password` = `confirm_password` THEN
            SET @sql = 'UPDATE br_user SET user_first_name = ?, user_last_name = ?, user_phone_number = ? WHERE user_id = ?'; -- user_username = ?, user_email = ?, 
            PREPARE stmt FROM @sql;
            EXECUTE stmt USING `first_name`, `last_name`, /*`username`, `email`,*/ `phone_number`, `id`;
            DEALLOCATE PREPARE stmt;
        /*ELSE
            SELECT 'Passwords do not match' AS ErrorMessage;
        END IF;
...
LOAD DATA INFILE'C:\Users\rfj594\Desktop\ZBC\Bogreden II\DB Jonas\Database\postnumre.csv'
INTO TABLE br_city
...
CALL insert_new_user_account('johndoe', 'john@example.com', 'password123', 'password123', 'Admin', 'John', 'Doe', '1234567890');
CALL insert_new_user_account('janesmith', 'jane@example.com', 'password456', 'password456', 'Employee', 'Jane', 'Smith', '9876543210');
CALL insert_new_user_account('alicej', 'alice@example.com', 'password789', 'password789', 'Employee', 'Alice', 'Johnson', '5555555555');
CALL insert_new_user_account('bobb', 'bob@example.com', 'passwordabc', 'passwordabc', 'Customer', 'Bob', 'Brown', '7777777777');
CALL insert_new_user_account('emmaw', 'emma@example.com', 'passwordxyz', 'passwordxyz', 'Customer', 'Emma', 'White', '9999999999');
CALL insert_new_user_account('michaell', 'michael@example.com', 'pass123word', 'pass123word', 'Customer', 'Michael', 'Lee', '1111111111');
CALL insert_new_user_account('saraht', 'sarah@example.com', 'pass456word', 'pass456word', NULL, 'Sarah', 'Taylor', '2222222222');
CALL insert_new_user_account('davidm', 'david@example.com', 'pass789word', 'pass789word', NULL, 'David', 'Martinez', '3333333333');
CALL insert_new_user_account('sophiag', 'sophia@example.com', 'passabcword', 'passabcword', NULL, 'Sophia', 'Garcia', '4444444444');
CALL insert_new_user_account('daniell', 'daniel@example.com', 'passxyzword', 'passxyzword', NULL, 'Daniel', 'Lopez', '6666666666');
